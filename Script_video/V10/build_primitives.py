...
#    compositeOpacity.AddPoint(200.0,opac);
    compositeOpacity.AddPoint(400.0,opac);
    volumeProperty.SetScalarOpacity(compositeOpacity)
    volumeProperty.SetUseClippedVoxelIntensity 	(200) 	

    funcOpacityGradient = vtk.vtkPiecewiseFunction()
    funcOpacityGradient.AddPoint(1,   0)
    funcOpacityGradient.AddPoint(5,   0)
    funcOpacityGradient.AddPoint(100,  0)
...
def build_actor_from_image2(path_source,r,g,b,opac,spec,diff,amb,isoVal,crop_type=0,reduced_data_type=0,path_alternative=None,slice_min=-1,slice_max=-1):
    ...
    if(decimation):
        ...
        decimator.PreSplitMeshOn()
        decimator.SplittingOn()
        decimator.SetDegree(5)
        decimator.SetBoundaryVertexDeletion(True)
        decimator.Update();
        geoBoneMapper = vtk.vtkPolyDataMapper()
        geoBoneMapper.SetInputConnection(decimator.GetOutputPort())
    elif (smooth):
        print("smoothing...")
        ...
        smoothFilter.Update()
        geoBoneMapper = vtk.vtkPolyDataMapper()
        geoBoneMapper.SetInputConnection(smoothFilter.GetOutputPort())
        print("smoothing ok")
    else:
        geoBoneMapper = vtk.vtkPolyDataMapper()
        geoBoneMapper.SetInputConnection(surface.GetOutputPort())
