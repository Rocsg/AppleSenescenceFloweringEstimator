...
def build_silhouette(file,renderer,actor,crop_type=0):
  ...
  #  r,g,b,   opac,spec,diff,amb ,  spec_r,spec_g,spec_b=1.0, 0.8,0.8 ,        0.27, 0.17 ,0.15,0.8           ,1.0 , 1.0  ,1.0
    ...
    actor= build_actor_from_image2(file,0.95,r,g,b,opac,spec,diff,amb,isoVal,crop_type,0,None)
...
def build_bois_sain(file,renderer,actor,crop_type=1):
...
def build_amadou(file,renderer,actor,crop_type=1):
    r,g,b=0.97,0.82, 0.6
    ...
    if(actor is None):
        ...
        actor.SetVisibility(False)
        renderer.RemoveActor(actor)
    actor=build_actor_from_image2(file,0,r,g,b,opac,spec,diff,amb,isoVal,crop_type,0,None)
